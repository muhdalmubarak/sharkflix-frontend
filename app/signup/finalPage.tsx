      ...
      if (!response.ok) {
        if (response.status === 409) {
          ...
          toast({
            ...
            action: (
              <Link href={loginUrl} className="bg-white text-black px-3 py-2 rounded-md hover:bg-gray-200">
                Login
              </Link>
            )
          });
      ...
      <Tabs.Root defaultValue="user" className="w-full">
        <Tabs.List className="flex space-x-2 mb-4">
          ...
          {!searchParams.get('refCode') && (
            <Tabs.Trigger value="creator" onClick={() => setRole('creator')} className="rounded-md hover:bg-gray-600 flex-1 text-center py-2 bg-gray-800 text-gray-300 radix-state-active:bg-[#e50914] radix-state-active:text-white data-[state=active]:bg-gray-600">
              Creator Signup
            </Tabs.Trigger>
          )}
          {!searchParams.get('refCode') && (
