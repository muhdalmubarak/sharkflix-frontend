...
export function VideoUploadDialog() {
  ...
  const [currentUser, setCurrentUser] = useState<any>(null); // Adjust the type based on your data structure
  ...
  const fetchUserData = async () => {
    ...
    setCurrentUser(data[0]); // Update state with fetched movies
  };
  ...
  const handleVideoChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      // Check if the size exceeds 50 MB
      // if (
      //   file.size > 50 * 1024 * 1024 &&
      //   !currentUser?.request_approved
      // ) {
      //   toast.error("File size exceeds 50 MB. Please contact the administrator for approval.\n+6011-3302-5077 Admin Sharkv")
  ...
  const uploadToSupabase = async (
    ...
    return (
      "https://zdcryvewjhsccriuzltq.supabase.co/storage/v1/object/public/" +
      data.fullPath
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const videoPath = `video_videohub/movie/${normalizedVideoName}`; // Path for video
      const thumbnailPath = `video_videohub/thumbnails/${normalizedThumbnailName}`; // Path for thumbnail

      // Upload video and thumbnail to Supabase Storage
      const videoUrl = await uploadToSupabase(videoFile, videoPath);
      const thumbnailUrl = await uploadToSupabase(thumbnailFile, thumbnailPath);
