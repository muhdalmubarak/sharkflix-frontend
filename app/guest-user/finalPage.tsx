...
async function payment(youtubeUrl: string, refCode: string, price: number) {
  try {
...
export default function FinalPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        const data = await postData("/api/video-get", { youtubeUrlLink });
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (!affCode) {
        await payment(youtubeUrlLink, "", movieData.price);
        return;
      }
      ...
      if (session?.user) {
        ...
        window.location.href = await PayHalalService.initiateVideoPayment({
          ...
          youtubeUrl: youtubeUrlLink,
          price: movieData.price
        });
      } else {
        // First check login status
        router.push(`/login?movie=${movieFileName}&refCode=${affCode}`);
      }
    } catch (error) {
